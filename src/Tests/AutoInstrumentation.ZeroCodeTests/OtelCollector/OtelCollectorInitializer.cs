...
public class OtelCollectorInitializer
{
    ...
    public static ReceivedSpan[] GetSpans(string serviceName)
    {
        return _otlpCollectorServer.ReceivedSpans
            ...
            .SelectMany(resource => resource.ScopeSpans
                .SelectMany(scope => scope.Spans
                    .Select(span => new ReceivedSpan(resource.Resource, scope.Scope, Span: span))))
            .ToArray();
    }

    public record ReceivedSpan(Resource Resource, InstrumentationScope Scope, Span Span);
}